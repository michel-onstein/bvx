...
public actor SpotlightIndexer {
    /// The domain everything vbx indexes belongs to, so it can all be removed
    /// at once when a different workspace is opened.
    private static let domain = "com.qjam.vbx.beads"
    ...
    public func index(_ issues: [Issue], workspace: String) async {
        ...
        let items = issues.map { issue -> CSSearchableItem in
            ...
            // The id and labels become keywords, so both "vbx-8ou" and the
            // label find the bead.
    ...
    /// Removes everything vbx has indexed.
