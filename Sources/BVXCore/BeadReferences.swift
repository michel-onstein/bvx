...
///   `whois-q1rfj` and one whose ids look like `bvx-3` both work, and a
...
public enum BeadReferences {
    ...
    ///
    /// The set is what bounds a token: `bvx-8ou,` yields `bvx-8ou` because the
    /// comma ends the run, while `bvx-8ou-old` yields one unknown token and so
    /// is left alone.
...
public enum BeadURL {
    ...
    /// `bvx://open?workspace=<path>&bead=<id>`, with either part optional.
    ...
    /// The bead id a bvx URL names, if it names one.
    public static func bead(in url: URL) -> String? {
        query(url, named: "bead")
    }

    /// The workspace path a bvx URL names, if it names one.
