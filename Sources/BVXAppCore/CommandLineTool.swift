...
/// Installs the bundled `bvx-cli` somewhere on the user's PATH.
///
...
public enum CommandLineTool {
    ...
    /// The `bvx-cli` shipped beside the app, if there is one.
    ///
    ...
    public static var bundledToolURL: URL? {
        ...
        if let executable {
            let candidate = executable.appendingPathComponent("bvx-cli")
        ...
        let sibling = URL(fileURLWithPath: CommandLine.arguments[0])
            ...
            .appendingPathComponent("bvx-cli")
    ...
    public static func install() -> InstallResult {
        guard let source = bundledToolURL else {
            return .failed("bvx-cli was not found beside the app.")
        }

        let panel = NSSavePanel()
        panel.title = "Install Command Line Tool"
        panel.message =
            "Choose a directory on your PATH. A link to the bundled bvx-cli is created there."
        panel.nameFieldStringValue = "bvx-cli"
