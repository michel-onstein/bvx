...
**vbx** is a native macOS implementation of [`bv` (Beads Viewer)](https://github.com/Dicklesworthstone/beads_viewer) —
a dependency-graph-aware viewer for **beads** issue stores.

Where `bv` is a Bubble Tea terminal UI, `vbx` is a SwiftUI/AppKit application: document-based,
...
and export. `vbx` **reuses that engine unmodified** — compiled with `go build -buildmode=c-archive`
...
1. [Design doc §1–4](VBX_DESIGN.md#1-summary) — what `bv` is and why the engine is reused
2. [§5–9](VBX_DESIGN.md#5-system-architecture) — architecture, bridge, model, data flow
3. [§10–12](VBX_DESIGN.md#10-user-interface-design) — the UI, graph rendering, search
4. [§13–17](VBX_DESIGN.md#13-robot-protocol-cli-and-automation) — agents, exports, performance, testing, distribution
5. [§18–20](VBX_DESIGN.md#18-delivery-plan) — plan, risks, open questions
