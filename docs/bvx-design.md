...
`bvx` delivers the same functionality as a **native macOS application** — SwiftUI/AppKit,
...
**The central architectural decision** is that `bvx` **reuses `bv`'s Go engine verbatim**,
...
hard-won analysis logic in one place, and lets `bvx` track upstream releases by bumping a
submodule instead of re-deriving algorithms. See [§4](#4-architecture-decision-how-bvx-gets-its-engine).
...
1. **Functional parity** with `bv` — every view, every metric, every export, every robot
   command, reachable from `bvx`.
2. **Numerical parity** — a metric shown in `bvx` is identical to the same metric in `bv`
...
1. **Not an editor.** `bvx` reads bead stores and shells out to `bd`/`br` for mutations,
...
3. **Not a terminal emulator.** `bvx` does not reproduce the TUI's character-cell layouts;
   it reproduces their *meaning* with native controls.
4. **No new analysis.** v1 adds no metrics `bv` does not have.

---

## 4. Architecture Decision: How `bvx` Gets Its Engine
...
flowchart TB
    ...
    subgraph B["Bridge"]
        ...
        CABI["C ABI<br/>bvx_open · bvx_call<br/>bvx_call_async · bvx_cancel"]:::bridge
    end
...
├── App/
│   ├── bvx/                      # the macOS app target
│   └── bvx-cli/                  # CLI target (robot protocol parity)
└── docs/
```
...
```c
typedef struct BvxHandle BvxHandle;          // opaque session
typedef struct { uint8_t *ptr; int64_t len; } BvxBuf;

// Lifecycle. config_json selects data source, workspace, cache dir, limits.
BvxHandle *bvx_open(const char *config_json, BvxBuf *err_out);
void       bvx_close(BvxHandle *h);

// Synchronous request/response. Returns 0 on success, non-zero error code.
int32_t    bvx_call(BvxHandle *h,
                    const char *method,       // e.g. "insights", "triage", "plan"
                    const uint8_t *req, int64_t req_len,
                    BvxBuf *out);             // caller frees with bvx_free

// Asynchronous: returns a token immediately; the process-wide callback fires on
// progress, completion, or cancellation.
int64_t    bvx_call_async(BvxHandle *h, const char *method,
                          const uint8_t *req, int64_t req_len);
int32_t    bvx_cancel(int64_t token);

typedef void (*BvxCallback)(int64_t token, int32_t code,
                            const uint8_t *payload, int64_t len);
void       bvx_set_callback(BvxCallback cb);

void       bvx_free(BvxBuf *buf);
```
...
callable from `bvx` with no C, no Go, and no build change.

### 6.2 Memory ownership

- Go allocates every response buffer with `C.malloc` and hands ownership to Swift.
- Swift copies into `Data` or decoded values, then calls `bvx_free` in a `defer`. This is
...
- Swift-owned request buffers are valid only for the duration of the call; the Go side
  copies before returning from `bvx_call_async`.
...
sequenceDiagram
    ...
    E->>B: bvx_call_async("insights", req)
    B-->>E: token
    ...
    S->>E: Task cancelled
    E->>B: bvx_cancel(token)
    B->>G: ctx cancelled
```

- Swift never blocks the main thread on the engine. `EngineActor` serialises access to a
  single `BvxHandle`; concurrency happens inside Go, where the existing worker pools and
...
- Swift structured-concurrency cancellation maps to `bvx_cancel`, which cancels the Go
...
`bvx` is a **document-based app**: a "document" is a bead workspace (a directory containing
...
cannot drift from `bv`. `bvx` adds a native folder picker when discovery finds nothing.
...
- Git history requires reading `.git`, which is inside the granted folder, so no extra
  ...
  **Decision:** for the sandboxed app, route correlation through a Go path that reads the
  object store directly; keep the subprocess path for `bvx-cli`, which is not sandboxed.
  This is the one place the engine needs an upstream-friendly patch, and it is worth
  contributing back.

---

## 9. Analysis Pipeline in `bvx`
...
- `bv` fails closed on robot claims when PageRank or betweenness are incomplete
  (`ClaimUnsafeReasons`); `bvx` mirrors this by disabling "claim this bead" actions with the
  same reason text.
...
| TUI mechanic | `bvx` native equivalent |
|---|---|
...
| `bv` view | `bvx` surface | Native technology | Notes |
|---|---|---|---|
...
`bv` is vim-keyed; macOS users expect `⌘`-keyed. `bvx` serves both:
...
approximation; `bvx` draws a real, interactive, hit-testable graph.
...
Search is also wired to **Spotlight**: `bvx` indexes open workspaces into
`CSSearchableIndex`, so a bead ID or title typed into Spotlight opens it directly in `bvx`.
...
1. **`bvx-cli`** — a separate executable target linking the same XCFramework, accepting the
   full `--robot-*` flag set and emitting byte-identical JSON/TOON. Shipped inside the app
   bundle at `Contents/MacOS/bvx-cli`, with an "Install Command Line Tool…" menu item that
   symlinks it into `/usr/local/bin` (the Xcode / `gh` pattern). A CI job diffs
   `bvx-cli --robot-X` against `bv --robot-X` across the fixture corpus.
2. **App Intents / Shortcuts** — `GetTriage`, `GetNextBead`, `GetExecutionPlan`, `GetAlerts`,
   `ForecastBead`, `ExportReport`. This makes `bvx` scriptable from Shortcuts, Spotlight, and
   Siri, and callable from other apps without shelling out.
3. **URL scheme** — `bvx://open?workspace=…&bead=…&view=graph` for deep links from commit
...
flowchart TB
    ...
    GUI["bvx.app (SwiftUI)"]:::s --> K["BeadsEngineKit"]:::e
    CLI["bvx-cli (robot flags)"]:::s --> K
    INT["App Intents / Shortcuts"]:::s --> K
    URL["bvx:// URL handler"]:::s --> K
...
- **Hooks** — user shell commands around export phases. In the sandboxed app these run via a
  ...
  `bvx-cli`; the CLI keeps full hook behaviour.
...
|---|---|---|
| Cold launch → window visible | < 300 ms | `bvx_open` on a background task; list paints from Phase-1 data |
...
metric, and why the `bvx` UI must be designed to render correctly without it.

Everything is instrumented with `os_signpost` around each engine call and layout pass, so
Instruments traces attribute time to a named phase. A `bvx --profile-startup` mode mirrors
`bv`'s.
...
flowchart LR
    ...
    P["<b>Parity tests</b><br/>bvx-cli vs bv:<br/>byte-diff robot JSON<br/>over the fixture corpus"]:::t
...
fuzzing; `bvx` reuses that corpus and asserts that for every fixture and every robot method,
`bvx-cli` output equals `bv` output modulo a documented, enumerated exception list (timing
...
`bvx_call` buffer is freed and that cancelling mid-flight neither leaks nor double-frees.
...
flowchart LR
    ...
    SPM --> APP["xcodebuild archive<br/>→ bvx.app, universal"]:::b
...
| **6 — Automation** (2 wks) | `bvx-cli` full robot flag set, App Intents, URL scheme, Spotlight | Full parity suite green across all robot methods |
...
1. **Write support.** `bv` is read-only and shells to `bd`/`br`. Should `bvx` v1 offer inline
...
Plus `bvx`-local methods that are not robot commands: `open`, `close`, `reload`, `data_hash`,
