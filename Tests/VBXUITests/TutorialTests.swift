...
struct TutorialTests {
    ...
    private func isolated() -> (Tutorial, UserDefaults) {
        let suite = UserDefaults(suiteName: "vbx.tutorial.test.\(UUID().uuidString)")!
    ...
    func staleProgressIsDropped() {
        let suite = UserDefaults(suiteName: "vbx.tutorial.test.\(UUID().uuidString)")!
        suite.set(["welcome", "a-section-that-no-longer-exists"], forKey: "vbx.tutorial.completed")
    ...
    func opensAtFirstUnread() {
        let suite = UserDefaults(suiteName: "vbx.tutorial.test.\(UUID().uuidString)")!
        suite.set([Tutorial.sections[0].id], forKey: "vbx.tutorial.completed")
