...
private var fixturePath: String {
    // Tests/BVXEngineTests/EngineTests.swift -> package root -> Fixtures/demo
...
func issuesDecode() async throws {
    ...
    let facade = try #require(issues.first { $0.id == "bvx-3" })
    ...
    #expect(facade.dependencies.contains { $0.dependsOnID == "bvx-2" })
...
func phase1Immediate() async throws {
    ...
    // bvx-3 is depended on by many other beads.
    #expect(metrics.blocks("bvx-3") >= 5)
...
func phase2Metrics() async throws {
    ...
    // bvx-3 blocks the most work, so it must carry the highest PageRank.
    let top = pageRank.max { $0.value < $1.value }
    #expect(top?.key == "bvx-3")
...
func unblocks() async throws {
    ...
    // Closing bvx-3 should unblock the views that wait on it.
    let freed = try await engine.unblocks("bvx-3")
    #expect(freed.contains("bvx-4"))
    #expect(freed.contains("bvx-5"))
...
func graphEdges() async throws {
    ...
    #expect(edges.contains { $0.from == "bvx-3" && $0.to == "bvx-2" })
...
func openMissingWorkspace() async {
    ...
    await #expect(throws: (any Error).self) {
        try await engine.open(path: NSTemporaryDirectory() + "/bvx-does-not-exist-\(UUID())")
    }
}
