...
///
/// `Resources/bvx.icns` is a build input, not a build product — `build-app.sh`
...
private var iconURL: URL {
    URL(fileURLWithPath: #filePath)
        ...
        .deletingLastPathComponent()
        .deletingLastPathComponent()
        .appendingPathComponent("Resources/bvx.icns")
}
...
private func representations() throws -> [Int: NSImageRep] {
    let image = try #require(NSImage(contentsOf: iconURL), "bvx.icns did not decode")
...
func iconHasEveryRepresentation() throws {
    #expect(
        ...
        "Resources/bvx.icns is missing — run ./scripts/build-icon.sh")
