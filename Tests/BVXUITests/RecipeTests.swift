...
struct RecipeTests {
    ...
    func saveAndDelete() async throws {
        ...
        var recipe = Recipe(name: "bvx-test-recipe", description: "temporary")
        recipe.sort = RecipeSort(field: "id", direction: "asc")
        await store.saveRecipe(recipe)

        #expect(
            store.recipes.userDefined.map(\.recipe.name).contains("bvx-test-recipe"),
            "save failed: \(store.loadError ?? "none")")

        await store.applyRecipe(named: "bvx-test-recipe")
        let ids = store.recipeIDs ?? []
        #expect(ids == ids.sorted())

        await store.deleteRecipe(named: "bvx-test-recipe")
        let remaining = store.recipes.userDefined.map(\.recipe.name)
        #expect(
            !remaining.contains("bvx-test-recipe"),
